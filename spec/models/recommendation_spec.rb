...
RSpec.describe Recommendation, :type => :model do
	...
	describe "validations" do
		...
		it "requiert un sender" do
			expect(Recommendation.new(:sender_id => nil, :receiver_id => @friend.id, :content => @content, :category => "film", :item => "un titre de film")).not_to be_valid
		end
		
		it "requiert un sender qui existe" do
			expect(Recommendation.new(:sender_id => 3, :receiver_id => @friend.id, :content => @content, :category => "film", :item => "un titre de film")).not_to be_valid
		end
		
		it "requiert un receiver" do
			expect(Recommendation.new(:sender_id => @user.id, :content => @content, :category => "film", :item => "un titre de film")).not_to be_valid
		end
		
		it "requiert un receiver qui existe" do
			expect(Recommendation.new(:sender_id => @user.id, :receiver_id => 3, :content => @content, :category => "film", :item => "un titre de film")).not_to be_valid
		end
		
		it "devrait rejeter un contenu trop long" do
			expect(Recommendation.new(:sender_id => @user.id, :receiver_id => @friend.id, :content => "a" * 201, :category => "film", :item => "un titre de film")).not_to be_valid
		end
		
		it "requiert une category valide" do
			expect(Recommendation.new(:sender_id => @user.id, :receiver_id => @friend.id, :content => "a", :category => "mauvaise cat", :item => "un titre de film")).not_to be_valid
		end
		
		it "requiert un item" do
			expect(Recommendation.new(:sender_id => @user.id, :receiver_id => @friend.id, :content => "a", :category => "mauvaise cat")).not_to be_valid
		end
