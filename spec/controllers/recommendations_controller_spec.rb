...
render_views
	...
	describe "POST 'create'" do
    	...
    	it "devrait créer une recommendation" do
        	expect { post :create, :recommendation => { :receiver_id => @friend, :content => "la recommendation", :category => "film", :item => "un titre de film" } }.to change(Recommendation, :count).by(1)
	...
	describe "DELETE 'destroy'" do
		...
		before(:each) do
      	...
      	@friend = FactoryGirl.create(:user, :email => "autree@gmail.com")
      	@user1.friendships.create(:friend_id => @friend.id, :status => "accepted")
      	post :create, :recommendation => { :receiver_id => @friend, :content => "la recommendation", :category => "film", :item => "un titre de film" }
